				...
				switch( stage )
				{
					case IlluminationStage.Ambient:
						...
						else
						{
							...
							if( pass.Ambient.CompareTo( Color.black ) != 0 ||
							    pass.Emissive.CompareTo( Color.black ) != 0 )
							{
								...
								newPass.Diffuse = Color.black;
								newPass.Specular = Color.black;

								// if ambient and emissive are zero, then color write isn't needed
								if( newPass.Ambient.CompareTo( Color.black ) == 0 &&
								    newPass.Emissive.CompareTo( Color.black ) == 0 )
								{
					...
					case IlluminationStage.PerLight:
						...
						else
						{
							...
							if( pass.LightingEnabled &&
							    ( pass.Diffuse.CompareTo( Color.black ) != 0 ||
							      pass.Specular.CompareTo( Color.black ) != 0 ) )
							{
								// copy existing pass
								Pass newPass = new Pass( this, pass.Index );
								pass.CopyTo( newPass );
