...
namespace Axiom.Serialization
{
	...
	/// <summary>
	/// The storage format of float values
	/// </summary>
	public enum floatStorageFormat
	{
		/// <summary>
		/// float is stored as float, reducing precision if you're using OGRE_DOUBLE_PRECISION
		/// </summary>
		Float,

		/// <summary>
		/// float as stored as double, not useful unless you're using OGRE_DOUBLE_PRECISION
		/// </summary>
		Double
	}
	...
	public class StreamSerializer : IDisposable
	{
		...
		protected floatStorageFormat mfloatFormat = floatStorageFormat.Float;
		...
		public Endian Endian { get { return mEndian; } }
		...
		public StreamSerializer( Stream stream, Endian endianMode, bool autoHeader )
			...
			: this( stream, endianMode, autoHeader, floatStorageFormat.Float )
#else
			: this( stream, endianMode, autoHeader, floatStorageFormat.Double )
#endif
		{}
		...
		public StreamSerializer( Stream stream, Endian endianMode, bool autoHeader, floatStorageFormat realFormat )
		{
			mStream = stream;
			mEndian = endianMode;
		...
		public Chunk ReadChunkBegin( uint id, UInt16 maxVersion, string msg )
		{
			...
			{
				Debug.Log( "Error: " + msg + " : Data version is " + c.version + " but this software can only read up to version " + maxVersion );
				// skip
				ReadChunkEnd( c.id );
				return null;
			}
		...
		protected void ReadHeader()
		{
			...
			mfloatFormat = realIsDouble ? floatStorageFormat.Double : floatStorageFormat.Float;
		...
		protected void WriteHeader()
		{
			...
			// real format
			bool realIsDouble = ( mfloatFormat == floatStorageFormat.Double );
			Write( realIsDouble );
