...
//     <id value="$Id: HardwareBuffer.cs 2940 2012-01-05 12:25:58Z borrillis $"/>
// </file>
...
namespace Axiom.Graphics
{
	...
	abstract public class HardwareBuffer : DisposableObject
	{
		...
		{
			UnityEngine.Debug.Assert( !IsLocked, "Cannot lock this buffer because it is already locked." );
			UnityEngine.Debug.Assert( offset >= 0 && ( offset + length ) <= sizeInBytes, "The data area to be locked exceeds the buffer." );
		...
		{
			UnityEngine.Debug.Assert( this.IsLocked, "Cannot unlock this buffer if it isn't locked to begin with." );
