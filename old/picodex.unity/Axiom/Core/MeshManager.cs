...
namespace Axiom.Core
{
	...
	public class MeshManager : ResourceManager, IManualResourceLoader
	{
		...
		/// </summary>
		public static MeshManager Instance { get { return Singleton<MeshManager>.Instance; } }
		...
		public Real BoundsPaddingFactor { get { return _boundsPaddingFactor; } set { _boundsPaddingFactor = value; } }
		...
		/// </summary>
		public event EventHandler<MeshSerializerArgs> ProcessMaterialName { add { _processMaterialNameEvent.EventSinks += value; } remove { _processMaterialNameEvent.EventSinks -= value; } }
		...
		///	finer control over resources.
		/// </summary>
		public event EventHandler<MeshSerializerArgs> ProcessSkeletonName { add { _processSkeletonNameEvent.EventSinks += value; } remove { _processSkeletonNameEvent.EventSinks -= value; } }
		...
		/// </summary>
		public bool PrepareAllMeshesForShadowVolumes { get { return _prepAllMeshesForShadowVolumes; } set { _prepAllMeshesForShadowVolumes = value; } }
		...
		public Mesh Load( string name, string group )
		{
		...
		{
			return CreateCurvedIllusionPlane( name, group, plane, width, height, curvature, xSegments, ySegments, normals, texCoordSetCount, xTiles, yTiles, upVector, Quaternion.Identity, BufferUsage.StaticWriteOnly, BufferUsage.StaticWriteOnly, true, true );
		}
		...
		public Mesh CreateCurvedIllusionPlane( string name, string group, Plane plane, float width, float height, float curvature, int xSegments, int ySegments, bool normals, int texCoordSetCount, float xTiles, float yTiles, Vector3 upVector, Quaternion orientation, BufferUsage vertexBufferUsage, BufferUsage indexBufferUsage, bool vertexShadowBuffer, bool indexShadowBuffer )
		{
			...
			GenerateCurvedIllusionPlaneVertexData( vertexBuffer, ySegments, xSegments, xSpace, halfWidth, ySpace, halfHeight, xform, firstTime, normals, orientation, cameraPosition, sphereRadius, xTiles, yTiles, numberOfTexCoordSets, ref min, ref max, ref maxSquaredLength );

			// generate face list
			subMesh.useSharedVertices = true;
			_tesselate2DMesh( subMesh, xSegments + 1, ySegments + 1, false, indexBufferUsage, indexShadowBuffer );
			...
			unsafe
			{
				...
				for( int y = 0; y <= ySegments; y++ )
				{
					for( int x = 0; x <= xSegments; x++ )
					{
                        ...
                        float diff_x = (x - ((float)xSegments / 2)) / (float)xSegments;
                        float diff_y = (y - ((float)ySegments / 2)) / (float)ySegments;
                        float dist = Utility.Sqrt(diff_x * diff_x + diff_y * diff_y);
		...
		new public Mesh this[ string name ] { get { return (Mesh)base[ name ]; } }

		public Mesh this[ ResourceHandle handle ] { get { return (Mesh)base[ handle ]; } }
