...
//     <id value="$Id: RenderSystem.cs 2940 2012-01-05 12:25:58Z borrillis $"/>
// </file>
...
namespace Axiom.Graphics
{
	...
	abstract public class RenderSystem : DisposableObject
	{
		...
		/// </summary>
		protected List<RenderTexture> prioritizedRenderTextures = new List<RenderTexture>();

		/// <summary>
		///		List of current render targets (i.e. a <see cref="RenderWindow"/>, or a<see cref="RenderTexture"/>)
		/// </summary>
		protected Dictionary<string, RenderTexture> renderTargets = new Dictionary<string, RenderTexture>();
		...
		/// </summary>
		protected Rect activeRect;

		/// <summary>
		///		Active render target.
		/// </summary>
		protected RenderTexture activeRenderTexture;
		...
		/// </summary>
		protected Matrix4x4[] worldMatrices = new Matrix4x4[256];
		...
		/// </summary>
		public Rect ActiveRect { get { return activeRect; } }
		...
		public int RenderTextureCount { get { return renderTargets.Count; } }
		...
		virtual public void AttachRenderTexture( RenderTexture target )
		{
			renderTargets.Add( target.Name, target );

			if( target.Priority == RenderTexturePriority.RenderToTexture )
			{
				// insert at the front of the list
				prioritizedRenderTextures.Insert( 0, target );
			}
			else
			{
				// add to the end
				prioritizedRenderTextures.Add( target );
			}
		}
		...
		public RenderTexture DetachRenderTexture( string name )
		{
			RenderTexture target = prioritizedRenderTextures.Find( delegate( RenderTexture item ) { return item.Name == name; } );

			return DetachRenderTexture( target );
		}
		...
		/// <returns>the render target that was detached</returns>
		virtual public RenderTexture DetachRenderTexture( RenderTexture target )
		{
			if( target != null )
			{
				prioritizedRenderTextures.Remove( target );
				renderTargets.Remove( target.Name );

				/// If detached render target is the active render target, 
				/// reset active render target
				if( target == activeRenderTexture )
				{
		...
		virtual public void DisableTextureUnit( int stage )
		{
			...
			SetTextureMatrix( stage, Matrix4x4.Identity );
		}
		...
		/// </summary>
		virtual public void InitRenderTextures()
		{
			// init stats for each render target
			foreach( KeyValuePair<string, RenderTexture> item in renderTargets )
			{
		...
		virtual public void SetWorldMatrices( Matrix4x4[] matrices, ushort count )
		{
		...
		virtual public void RemoveRenderTextures()
		{
			// destroy each render window
			RenderTexture primary = null;
			while( renderTargets.Count > 0 )
			{
				Dictionary<string, RenderTexture>.Enumerator iter = renderTargets.GetEnumerator();
				iter.MoveNext();
				KeyValuePair<string, RenderTexture> item = iter.Current;
				RenderTexture target = item.Value;
				...
				//else
				//{
				DetachRenderTexture( target );
				target.Dispose();
				//}
			}
			if( primary != null )
			{
				DetachRenderTexture( primary );
				primary.Dispose();
			}

			renderTargets.Clear();
			prioritizedRenderTextures.Clear();
		}
		...
		/// </summary>
		virtual public void UpdateAllRenderTextures()
		{
			this.UpdateAllRenderTextures( true );
		}
		...
		virtual public void UpdateAllRenderTextures( bool swapBuffers )
		{
			// Update all in order of priority
			// This ensures render-to-texture targets get updated before render windows
			foreach( RenderTexture target in prioritizedRenderTextures )
			{
		...
		/// if <see cref="UpdateAllRenderTextures"/> was called with a 'false' parameter.
		/// </summary>
		/// <param name="swapBuffers"></param>
		virtual public void SwapAllRenderTextureBuffers( bool waitForVSync )
		{
			// Update all in order of priority
			// This ensures render-to-texture targets get updated before render windows
			foreach( RenderTexture target in prioritizedRenderTextures )
		...
		///	</summary>
		abstract public Matrix4x4 ProjectionMatrix { get; set; }
		...
		///	</summary>
		abstract public Matrix4x4 ViewMatrix { get; set; }

		/// <summary>
		///		Gets/Sets the current world matrix.
		/// </summary>
		abstract public Matrix4x4 WorldMatrix { get; set; }
		...
		abstract public void ApplyObliqueDepthProjection( ref Matrix4x4 projMatrix, Plane plane, bool forGpuProgram );
		...
		/// <summary>
		/// Create a MultiRenderTexture, which is a render target that renders to multiple RenderTextures at once.
		/// </summary>
		/// <Remarks>
		/// Surfaces can be bound and unbound at will. This fails if Capabilities.MultiRenderTexturesCount is smaller than 2.
		/// </Remarks>
		/// <returns></returns>
		abstract public MultiRenderTexture CreateMultiRenderTexture( string name );
		...
		///		back a generic Matrix4x4 for storage in the engine.
		///	 </remarks>
		...
		/// <returns></returns>
		abstract public Matrix4x4 MakeOrthoMatrix( float fov, float aspectRatio, float near, float far, bool forGpuPrograms );
		...
		/// <param name="forGpuProgram"></param>
		/// <returns></returns>
		abstract public Matrix4x4 ConvertProjectionMatrix( Matrix4x4 matrix, bool forGpuProgram );
		...
		/// <param name="forGpuProgram"></param>
		/// <returns></returns>
		abstract public Matrix4x4 MakeProjectionMatrix( float fov, float aspectRatio, float near, float far, bool forGpuProgram );
		...
		abstract public void SetTextureMatrix( int stage, Matrix4x4 xform );
		...
		abstract public void SetRect( Rect viewport );
		...
		{
			RenderTexture rt = DetachRenderTexture( name );
			rt.Dispose();
		}
		...
		/// <param name="matrix"></param>
		/// <returns></returns>
		public Matrix4x4 ConvertProjectionMatrix( Matrix4x4 matrix )
		{
		...
		/// <param name="far">Far clipping plane distance.</param>
		/// <returns></returns>
		public Matrix4x4 MakeProjectionMatrix( float fov, float aspectRatio, float near, float far )
		{
			// create without consideration for Gpu programs by default
		...
		/// <remarks>Rect coordinates are in camera coordinate frame, i.e. camera is at the origin.</remarks>
		/// </summary>
		...
		/// <param name="forGpuProgram"></param>
		abstract public Matrix4x4 MakeProjectionMatrix( float left, float right, float bottom, float top, float nearPlane, float farPlane, bool forGpuProgram );
		...
		/// <param name="nearPlane"></param>
		/// <param name="farPlane"></param>
		public Matrix4x4 MakeProjectionMatrix( float left, float right, float bottom, float top, float nearPlane, float farPlane )
		{
		...
		/// <param name="far">Far clipping plane distance.</param>
		/// <returns></returns>
		public Matrix4x4 MakeOrthoMatrix( float fov, float aspectRatio, float near, float far )
		{
