...
namespace Axiom.Core
{
	...
	public class MeterManager
	{
		...
		private static void DebugAddEvent( TimingMeter meter, MeterEvent evt )
		{
			...
			{
				UnityEngine.Debug.Assert( debugMeterStack.Count > 0, "Meter stack is empty during ekExit" );
				MeterStackEntry s = debugMeterStack[ debugMeterStack.Count - 1 ];
				UnityEngine.Debug.Assert( s.meter == meter, "Entered " + s.meter.title + "; Exiting " + meter.title );
		...
		protected MeterManager()
		{
			...
			microsecondsPerTick = 1000000.0f / (float)System.Diagnostics.Stopwatch.Frequency;
			instance = this;
		}
		...
		protected TimingMeter GetMeterById( int id )
		{
			...
			UnityEngine.Debug.Assert( meter != null, string.Format( "Meter for id {0} is not in the index", id ) );
			return meter;
		}
		...
		protected void DumpEventLog()
		{
			...
			for ( int i = 0; i < eventTrace.Count; i++ )
			{
				...
				{
					UnityEngine.Debug.Assert( meterStack.Count > 0, "Meter stack is empty during ekExit" );
		...
		protected void GenerateReport( StreamWriter writer, int start, Dictionary<string, string> options )
		{
			...
			for ( int i = 0; i < eventTrace.Count; i++ )
			{
				...
				else if ( kind == ekExit )
				{
					...
					MeterStackEntry s = meterStack[ meterStack.Count - 1 ];
					meter.stackDepth--;
					UnityEngine.Debug.Assert( s.meter == meter );
