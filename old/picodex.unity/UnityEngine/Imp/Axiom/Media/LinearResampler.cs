...
namespace Axiom.Media
{
	...
	public partial class LinearResampler
	{
			...
			for( int z = dst.Front; z < dst.Back; z++, sz_48 += stepz )
			{
				...
				for( int y = dst.Top; y < dst.Bottom; y++, sy_48 += stepy )
				{
					...
					for( int x = dst.Left; x < dst.Right; x++, sx_48 += stepx )
					{
						...
						UnityEngine.Color x1y1z1 = UnityEngine.Color.White, x2y1z1 = UnityEngine.Color.White, x1y2z1 = UnityEngine.Color.White, x2y2z1 = UnityEngine.Color.White;
						UnityEngine.Color x1y1z2 = UnityEngine.Color.White, x2y1z2 = UnityEngine.Color.White, x1y2z2 = UnityEngine.Color.White, x2y2z2 = UnityEngine.Color.White;
		...
		private void Unpack( ref UnityEngine.Color dst, int x, int y, int z, PixelFormat format, IntPtr src, PixelBox srcbox, int elemsize )
		{
			unsafe
			{
				byte* pSrc = (byte*)src;
