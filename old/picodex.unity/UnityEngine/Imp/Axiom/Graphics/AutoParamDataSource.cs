...
//     <id value="$Id: AutoParamDataSource.cs 2940 2012-01-05 12:25:58Z borrillis $"/>
// </file>
...
namespace Axiom.Graphics
{
	...
	public class AutoParamDataSource : DisposableObject
	{
		...
		/// </summary>
        protected RenderTexture currentRenderTarget;
		...
		/// </summary>
        protected Matrix4x4 viewProjMatrix;

		/// <summary>
		///    Array of world matrices for the current renderable.
		/// </summary>
        protected Matrix4x4[] worldMatrix = new Matrix4x4[256];
		...
		/// </summary>
        protected Matrix4x4 worldViewMatrix;

		/// <summary>
		///    Current concatenated world, view, and projection matrices.
		/// </summary>
        protected Matrix4x4 worldViewProjMatrix;
		...
		/// </summary>
        protected Matrix4x4 inverseWorldViewMatrix;

		/// <summary>
		///    Inverse of the current view matrix.
		/// </summary>
        protected Matrix4x4 inverseViewMatrix;

		/// <summary>
		///    Inverse Transpose of the current world view matrix.
		/// </summary>
        protected Matrix4x4 inverseTransposeWorldViewMatrix;

		/// <summary>
		///		Current texture view projection matrix.
		/// </summary>
        protected Matrix4x4 textureViewProjMatrix;
		...
		protected Matrix4x4 ProjectionClipSpace2DToImageSpacePerspective = new Matrix4x4(
			//from ogre
			//0.5f,    0,  0, 0.5f, 
			//0, -0.5f,  0, 0.5f, 
			//0,    0,  0.5f,   0.5f,
		...
		protected override void dispose( bool disposeManagedResources )
		{
			if( !this.IsDisposed )
			{
				if( disposeManagedResources )
				{
					...
					{
                        UnityEngine.Object.Destroy(this.blankLight);
		...
		/// </summary>
        public RenderTexture RenderTexture { get { return currentRenderTarget; } set { currentRenderTarget = value; } }

		/// <summary>
		///		Get/Set the current active viewport in use.
		/// </summary>
		public Rect Viewport { get { return currentViewport; } set { currentViewport = value; } }

		/// <summary>
		///    Gets/Sets the current global ambient light color.
		/// </summary>
		public Color AmbientLight { get { return ambientLight; } set { ambientLight = value; } }
		...
		/// </summary>
		public Matrix4x4 InverseWorldViewMatrix
		{
			get
			{
		...
		/// </summary>
		public Matrix4x4 InverseViewMatrix { get { return this.ViewMatrix.Inverse(); } }

		/// <summary>
		///    Gets/Sets the inverse, transpose of current concatenated view matrices.
		/// </summary>
		public Matrix4x4 InverseTransposeViewMatrix { get { return this.InverseViewMatrix.Transpose(); } }

		/// <summary>
		///    Gets/Sets the inverse of current concatenated world and view matrices.
		/// </summary>
		public Matrix4x4 InverseTransposeWorldViewMatrix
		{
			get
			{
		...
		public Vector4 CameraPositionObjectSpace
		{
			get
			{
				...
				{
					cameraPositionObjectSpace = (Vector4)( this.InverseWorldMatrix * camera.transform.position );
		...
		/// </summary>
        public Vector3 CameraPosition { get { return camera.transform.position; } }
		...
		public Matrix4x4 ProjectionMatrix
		{
			get
			{
                ...
                //        projectionMatrix =
                //            Root.Instance.RenderSystem.ConvertProjectionMatrix( Matrix4x4.Identity, true );
                //    }
                //    else
                //    {
                //        projectionMatrix = camera.ProjectionMatrixRSDepth;
                //    }
                //    if( currentRenderTexture != null && currentRenderTexture.RequiresTextureFlipping )
                //    {
		...
		/// </summary>
		public Matrix4x4 TextureViewProjectionMatrix
		{
			get
			{
		...
		/// </summary>
		public Vector3 ViewDirection { get { return camera.transform.forward; } }

		/// <summary>
		/// Get the derived camera right vector (which includes any parent sceneNode transforms)
		/// </summary>
        public Vector3 ViewSideVector { get { return camera.transform.right; } }

		/// <summary>
		/// Get the derived camera up vector (which includes any parent sceneNode transforms)
		/// </summary>
        public Vector3 ViewUpVector { get { return camera.transform.up; } }

		public float NearClipDistance { get { return camera.nearClipPlane; } }

		public float FarClipDistance { get { return camera.farClipPlane; } }

        public float Time { get { return (float)UnityEngine.Time.time; } }
