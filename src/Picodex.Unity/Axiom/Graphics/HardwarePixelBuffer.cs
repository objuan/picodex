...
namespace Axiom.Graphics
{
	...
	abstract public class HardwarePixelBuffer : HardwareBuffer
	{
		...
		public PixelBox CurrentLock
		{
			get
			{
				UnityEngine.Debug.Assert( IsLocked, "Cannot get current lock: buffer not locked" );
				return _currentLock;
			}
		...
		virtual public PixelBox Lock( BasicBox lockBox, BufferLocking options )
		{
			...
			{
				UnityEngine.Debug.Assert( !isLocked );
		...
		{
			UnityEngine.Debug.Assert( !IsLocked, "Cannot lock this buffer, it is already locked!" );
			UnityEngine.Debug.Assert( offset == 0 && length == sizeInBytes, "Cannot lock memory region, must lock box or entire buffer" );
