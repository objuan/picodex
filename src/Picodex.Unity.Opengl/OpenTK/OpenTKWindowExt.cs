...
namespace Axiom.RenderSystems.OpenGL
{
	...
	/// </summary>
	public class OpenTKWindowExt : RenderWindow
	{
		#region Fields
		...
		/// </summary>
		/// <returns></returns>
		public override bool IsClosed { get { return _window == null && glContext == null; } }
			...
			if( miscParams != null )
			{
				foreach( KeyValuePair<string, object> entry in miscParams )
				{
					switch( entry.Key )
					{
                        ...
                        //    //OpenTKExternalWindow win = (OpenTKExternalWindow)entry.Value;

                        //    //glContext = new OpenTKGLContext(win.GraphicsMode, win.WindowInfo);

                        //    break;

						case "externalWindowInfo":
                            GraphicsMode mode = (GraphicsMode)miscParams["externalGraphicsMode"];
							glContext = new OpenTKGLContext(mode, (OpenTK.Platform.IWindowInfo)entry.Value );
							break;
