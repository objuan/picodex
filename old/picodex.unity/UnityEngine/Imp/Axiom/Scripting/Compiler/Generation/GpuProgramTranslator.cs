							...
							case Keywords.ID_PARAM_NAMED:
							{
								if( prop.Values.Count >= 3 )
								{
									...
									else
									{
										...
										bool isValid = true;
										GpuProgramParameters.ElementType type = GpuProgramParameters.ElementType.float;
										int count = 0;
										if( atom1.Value.Contains( "float" ) )
										{
											type = GpuProgramParameters.ElementType.float;
								...
								if( prop.Values.Count >= 2 )
								{
									...
									if( defFound )
									{
										switch( def.DataType )
										{
											...
											case GpuProgramParameters.AutoConstantDataType.float:
												    ...
												    def.AutoConstantType == GpuProgramParameters.AutoConstantType.FrameTime )
												{
													...
													try
													{
														...
														if( named )
														{
															/*parameters->setNamedAutoConstantfloat(name, def->acType, f);*/
														}
														else
														{
													...
													if( i2 != null )
													{
														...
														if( getfloat( i2, out extraInfo ) )
														{
															try
															{
																...
																if( named )
																{
																	/*parameters->setNamedAutoConstantfloat(name, def->acType, extraInfo);*/
																}
																else
																{
																	parameters.SetAutoConstant( index, def.AutoConstantType, extraInfo );
...
//#warning this need GpuProgramParametersShared implementation
			...
			protected void _translateHighLevelGpuProgram( ScriptCompiler compiler, ObjectAbstractNode obj )
			{
				...
				foreach( KeyValuePair<string, string> currentParam in customParameters )
				{
					...
					{
						Debug.Log( "Error in program {0} parameter {1} is not valid.", source, param );
					}
				}
...
//#warning this need GpuProgramParametersShared implementation
			...
			protected void _translateUnifiedGpuProgram( ScriptCompiler compiler, ObjectAbstractNode obj )
			{
				...
				foreach( KeyValuePair<string, string> currentParam in customParameters )
				{
					...
					if( !prog.SetParam( param, val ) )
					{
						Debug.Log( "Error in program {0} parameter {1} is not valid.", string.Empty, param );
					}
				}
