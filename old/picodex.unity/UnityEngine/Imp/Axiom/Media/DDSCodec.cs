...
namespace Axiom.Media
{
    ...
    public partial class DDSCodec : ImageCodec, IPlugin
    {
		...
		private void UnpackDXTColour(PixelFormat pf, DXTColorBlock block, UnityEngine.Color[] pCol)
    	{
    		// Note - we assume all values have already been endian swapped

    		// Colour lookup table
    		UnityEngine.Color[] derivedColours = new UnityEngine.Color[4];
    		...
    		if ( pf == PixelFormat.DXT1 && block.Color0 <= block.Color1 )
    		{
    			...
    			// transparent colour
    			derivedColours[ 3 ] = UnityEngine.Color.Black;
    		}
    		else
    		{
    			derivedColours[ 0 ] = PixelConverter.UnpackColor( PixelFormat.R5G6B5, color0 );
    		...
    		for ( int row = 0; row < 4; ++row )
    		{
    			for ( int x = 0; x < 4; ++x )
    			{
    				...
    				else
    				{
    					...
    					UnityEngine.Color col = pCol[ ( row * 4 ) + x ];
        ...
        private void UnpackDXTAlpha( DXTExplicitAlphaBlock block, UnityEngine.Color[] pCol )
        {
			// Note - we assume all values have already been endian swapped
    		int colorIndex = 0;
			...
			for (int row = 0; row < 4; ++row)
			{
				for (int x = 0; x < 4; ++x)
				{
					...
					// Convert to [0,1]
					pCol[colorIndex++].a = (float)val / 0xF;
				}
			}
        }

        /// Unpack DXT alphas into array of 16 colour values
		private void UnpackDXTAlpha(DXTInterpolatedAlphaBlock block, UnityEngine.Color[] pCol)
        {
        	// 8 derived alpha values to be indexed
        	float[] derivedAlphas = new float[8];

        	// Explicit extremes
        	derivedAlphas[ 0 ] = block.Alpha0 / (float)0xFF;
        	derivedAlphas[ 1 ] = block.Alpha1 / (float)0xFF;
                ...
                for ( int mip = 0; mip <= imgData.numMipMaps; mip++ )
                {
                    ...
                    if ( PixelUtil.IsCompressed( sourceFormat ) )
                    {
                        ...
                        if ( decompressDXT )
                        {
                            ...
                            UnityEngine.Color[] tempColors = new UnityEngine.Color[ 16 ];
