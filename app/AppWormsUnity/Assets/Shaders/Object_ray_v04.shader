	...
	SubShader{
		...
		Pass{
			...
			float4 frag(appdata_vxcm_object i) : COLOR
			{
				...
				//vxcm_getPixelRayLight(i, rayDirTex, rayOriginTex);
				//vxcm_getPixelRay(i, rayDirTex, rayOriginTex);
				rayOriginTex = i.uvw;

				float4 worldSpaceCameraPos = multInverse(UNITY_MATRIX_MV, float4(0, 0, 0, 1));

				rayDirTex = normalize(i.posW.xyz / i.posW.w - worldSpaceCameraPos);
	...
	SubShader{
			...
			Pass{
				...
				//	vxcm_getPixelRay(i, rayDirTex, rayOriginTex);
					...
					// MODO 2, con le coordinate world
					//rayDirTex = normalize(i.posW.xyz / i.posW.w - _WorldSpaceCameraPos);
					
					// MODO 3, screen

					// GET THE CURRENT CAMERA/LIGHT POSITION IN WORLD COORDINATE

					float4 worldSpaceCameraPos = multInverse(UNITY_MATRIX_V, float4(0, 0, 0, 1));
					//float4 worldSpaceCameraPos =  mul(transpose(UNITY_MATRIX_IT_MV), float4(0, 0, 0, 1));

					rayDirTex = normalize(i.posW.xyz / i.posW.w - worldSpaceCameraPos.xyz);
				//	rayDirTex = normalize(i.posW.xyz / i.posW.w - _WorldSpaceCameraPos);

					//float2 wcoord = (i.scrPos.xy / i.scrPos.w);
					//float2 v_position = wcoord * 2.0 - 1.0; // to projection

					//// in camera space
					//float3 viewDirVS = float3(v_position  * u_cameraInfo.xy, u_cameraInfo.z);
					////float3 viewDirVS = mul(u_prjInvTrx, float4(v_position, u_cameraInfo.z, 1));
