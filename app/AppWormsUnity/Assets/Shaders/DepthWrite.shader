...
Shader "Custom/DepthWrite" {
	...
	_Glossiness("Smoothness", Range(0,1)) = 0.5
		...
		_DepthOffset("_DepthOffset", Range(-1,1)) = 0  // DEPTH WRITE TWEAK
	}
		SubShader{
...
#define WorldNormalVector(data,normal) normal
		...
		sampler2D _MainTex;
	float _DepthOffset;  // DEPTH WRITE TWEAK
	...
	void frag_surf(v2f_surf IN, out fixed4 outColor : SV_Target, out float outDepth : SV_Depth) { // DEPTH WRITE TWEAK
		...
		float3 worldPos = IN.worldPos.xyz; // DEPTH WRITE TWEAK (take actual worldPos part explicitly using xyz swizzling mask)
		...
		//return c;
		outColor = c;
		float depthWithOffset = IN.worldPos.w + _DepthOffset;
